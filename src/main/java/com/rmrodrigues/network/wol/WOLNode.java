...
//            log.error("An error has occurred while sending the magic package.", e);
...
//            log.debug("validate():start");
    ...
    private void send() throws Exception {
        ...
        try {
            ...
            System.out.println("An error has occurred while sending the datagram.");
    ...
    public static void main(String[] args) {
        ...
        if (length == 0 || length > 3) {
            System.out.println("参数说明：Mac地址 [广播地址] [端口]");
            System.out.println("Parameter description: Mac address [broadcast address] [port]");
            return;
        }
        String macAddress = args[0];
        String broadcastIP = WOLNode.BROADCAST_IP;
        int port = WOLNode.WOL_PORT;
        if (length == 2) {
            broadcastIP = args[1];
        }
        if (length == 3) {
        ...
        WOLNode node = new WOLNode(macAddress, broadcastIP, port);
        try {
            node.wakeUP();
